...
#   sbx-ide set-default <cursor|vscode|codex>    set the default IDE target
#   sbx-ide set-default --agent <name> [--cursor|--vscode|--codex]
...
${C_BOLD}USAGE${C_RESET}
  ...
  $PROG <cursor|vscode|codex>            Persist the default IDE target.
  $PROG --agent <name> [--cursor|--vscode|--codex]
                                         Persist the sbx agent the sandbox runs
                                         (scoped to one IDE, or all if no scope).

${C_BOLD}NOTES${C_RESET}
  • Target resolution:  --cursor/--vscode/--codex flag > \$SBX_IDE_TARGET > config > cursor
...
EOF
  ...
  # --- set the agent -------------------------------------------------------
  if [[ -n "$agent" ]]; then
    [[ -z "$target_arg" ]] || die "with --agent, scope with --cursor/--vscode/--codex (not a bare '$target_arg')"
    local known=0 a
    ...
    if [[ -n "$scope" ]]; then
      ...
      local _t
      for _t in $SBX_IDE_TARGETS; do config_set "agent_$_t" "$agent"; done
  ...
  if [[ -n "$target_arg" ]]; then
    [[ -z "$scope" ]] || die "a bare target and --cursor/--vscode/--codex can't be combined; use one."
    ...
    hint "written to: $(config_file)"
    hint "Override per-run with --cursor/--vscode/--codex or \$SBX_IDE_TARGET."
    return 0
  fi
  ...
  hint "Set target: $PROG cursor|vscode|codex    ·    set agent: $PROG --agent <name> [--vscode]"
}
